...
// Command to move robot forward by specified steps
...
// Brief: Command to move selected robots forward by a given number of steps.
public class ForwardCommand implements Command {
    private final Server server;

    public ForwardCommand(Server server) {
        this.server = server;
    }
    ...
    public String execute(CommandRequest request, Socket clientSocket) {
        ...
        for (int i = 0; i < selectedRobots.size(); i++) {
            ...
            response.append("[").append(robot.getName()).append("] Moved forward ").append(moved).append(" steps.");
            if (blockReason != null) {
